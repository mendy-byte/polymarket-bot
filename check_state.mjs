...
// Open positions
const [open] = await conn.execute("SELECT COUNT(*) as cnt, SUM(cost_basis) as total FROM positions WHERE status='open'");
console.log('Open positions:', open[0].cnt, '| Total deployed:', open[0].total);

// Resolved positions
const [resolved] = await conn.execute("SELECT COUNT(*) as cnt, SUM(CASE WHEN pnl > 0 THEN 1 ELSE 0 END) as wins, SUM(CASE WHEN pnl <= 0 THEN 1 ELSE 0 END) as losses FROM positions WHERE status='resolved'");
console.log('Resolved:', resolved[0].cnt, '| Wins:', resolved[0].wins, '| Losses:', resolved[0].losses);
